...
export class MyDraw {
    ...
    static saturatedColor(r) {
        const lerp  = (a, b, t) => a + t * (b - a);
        let hue;
        
        if (r <= 1.0) { // 区間 A: r = 0.5 → 1.0
            const t = (r - 0.5) / 0.5;
            hue = lerp(0.66, 0.16, t);                  // t=0.0 → 1.0  色：0.66（青） → 0.16（黄）
        }
        else { // 区間 B: r = 1.0 → 2.0
            const t = (r - 1.0) / 1.0;  // 0 → 1
            if (t < 0.5) {
                hue = lerp(0.16, 0.0, t * 2);           // t=0.0 → 0.5  色：0.16（黄）→ 1.0（赤）
            } else {
                hue = lerp(1.0, 0.83, (t - 0.5) * 2);   // t=0.5→1.0 色：1.0（赤） → 0.83（紫）
            }
        }
        // console.log(`saturatedColor r:${r}, hue:${hue}`);
        return this.hsvToColor3(hue, 1.0, 1.0);
    }

    static hsvToColor3(h, s, v) {
        ...
        // console.log(`hsvToColor3 h:${h}, i:${i}, r:${r}, g:${g}, b:${b}`);
